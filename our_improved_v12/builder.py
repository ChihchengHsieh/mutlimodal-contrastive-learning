...
class OurImproved_v12(nn.Module):
    """
    From alt->improve
    ...
    def __init__(
        ...
        super(OurImproved_v12, self).__init__()
    ...
    def forward(self, img, tab):
        ...
        zqi1, zqi2, zi3, zio = (
            ...
            self.clip_img_pj(self.img_enc(i3)),
            self.clip_img_pj(self.img_enc(img)),
        )

        zqt1, zqt2, zt3, zto = (
            self.tab_pred(self.tab_pj(self.tab_auto.enc(t1))),
            self.tab_pred(self.tab_pj(self.tab_auto.enc(t2))),
            self.clip_tab_pj(self.tab_auto.enc(t3)),
            self.clip_tab_pj(self.tab_auto.enc(tab)),
        )
        ...
        loss = (img_loss + tab_loss + cross_l + auto_l) / 4
