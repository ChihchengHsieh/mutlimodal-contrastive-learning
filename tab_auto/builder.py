...
class TabAutoEncoder(nn.Module):
    def __init__(
        self, input_dim=10, cat_dim=1, n_tab_layers=5, dim=512, p=0.3, *args, **kwargs
    ) -> None:
        super().__init__(*args, **kwargs)
        ...
        self.criterion = AutoEncoderLoss_v2(
            n_cat_features=cat_dim,
        )
    ...
    def forward(self, x):
        ...
        out_drop = self.dec(self.enc(F.dropout(x, p=self.p)))

        # sigmoid gender
        out[:, -1] = torch.sigmoid(out[:, -1])
        out_drop[:, -1] = torch.sigmoid(out_drop[:, -1])
        loss = self.criterion(x, out)
        loss_drop = self.criterion(x, out_drop)
        return loss + loss_drop
