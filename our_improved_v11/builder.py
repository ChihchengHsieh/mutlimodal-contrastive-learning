...
class OurImproved_v11(nn.Module):
    """
    From alt->improve
    ...
    def __init__(
        ...
        super(OurImproved_v11, self).__init__()
        ...
        self.moco_img_pj = SimCLRProjectionHead(
            dim,
            dim,
            pred_dim,
        )
        ...
        self.moco_m_img_pj = SimCLRProjectionHead(
            dim,
            dim,
            pred_dim,
        )
        ...
        self.moco_m_tab_auto = TabAutoEncoder(
            input_dim=clinical_input_dim,
            dim=dim,
            n_tab_layers=n_tab_layers,
        )

        self.moco_tab_pj = SimCLRProjectionHead(
            dim,
            dim,
            pred_dim,
        )
        ...
        self.simclr_img_loss_fn = SimCLRLoss(temperature=0.1)  # Compare to SimCLR?
        self.simclr_tab_loss_fn = SimCLRLoss(temperature=0.1)
        self.moco_img_loss_fn = MoCoLoss()
        self.moco_tab_loss_fn = MoCoLoss()
    ...
    def forward(self, img, tab):
        ...
        moco_zi1, simclr_zi1, simclr_zi2, zi3, zio = (
            self.moco_img_pj(hi1),
            self.simclr_img_pj(hi1),
            self.simclr_img_pj(self.img_enc(i2)),
            self.cross_img_pj(self.img_enc(i3)),
            self.cross_img_pj(self.img_enc(img)),
        )

        t1 = self.tab_auto(F.dropout(tab, p=self.tab_drop_p))
        t2 = self.tab_auto(F.dropout(tab, p=self.tab_drop_p))
        t3 = self.tab_auto(F.dropout(tab, p=self.tab_drop_p))

        ht1 = self.tab_auto.enc(t1)

        moco_zt1, simclr_zt1, simclr_zt2, zt3, zto = (
            self.moco_tab_pj(ht1),
            self.simclr_tab_pj(ht1),
            self.simclr_tab_pj(self.tab_auto.enc(t2)),
        ...
        with torch.no_grad():
            ...
            self._update_momentum_encoders(self.moco_img_pj, self.moco_m_img_pj, self.m)
            moco_zi2 = self.moco_m_img_pj(self.m_img_enc(i2))

            self._update_momentum_encoders(self.tab_auto, self.moco_m_tab_auto, self.m)
            self._update_momentum_encoders(self.moco_tab_pj, self.m_tab_pj, self.m)
            moco_zt2 = self.m_tab_pj(self.moco_m_tab_auto.enc(t2))

        moco_img_l = self.moco_img_loss_fn(moco_zi1, moco_zi2)
        moco_tab_l = self.moco_tab_loss_fn(moco_zt1, moco_zt2)

        simclr_img_l = self.simclr_img_loss_fn(simclr_zi1, simclr_zi2)
        simclr_tab_l = self.simclr_tab_loss_fn(simclr_zt1, simclr_zt2)

        cross_l = (self.cross_loss_fn(zio, zto) + self.cross_loss_fn(zi3, zt3)) / 2
        auto_l = self.auto_loss_fn(tab, t1)

        loss = (
            moco_img_l + moco_tab_l + simclr_img_l + simclr_tab_l + cross_l + auto_l
        ) / 4
        return loss, {
            "moco_img_l": moco_img_l.item(),
            "moco_tab_l": moco_tab_l.item(),
            "simclr_img_l": simclr_img_l.item(),
            "simclr_tab_l": simclr_tab_l.item(),
