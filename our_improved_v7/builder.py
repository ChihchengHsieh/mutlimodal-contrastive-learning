...
class OurImproved_v7(nn.Module):
    """
    From alt->improve
    ...
    def __init__(
        ...
        super(OurImproved_v7, self).__init__()
    ...
    def forward(self, img, tab):
        i1, i2 = self.augment1(img), self.augment2(img)
        ...
        li = self.ci * self._get_comb_loss(zs=[zi1, zi2, zio], loss_fn=self.img_loss)
        lt = self.ct * self._get_comb_loss(zs=[zt1, zt2, zto], loss_fn=self.tab_loss)
        lc = self.cit * self._get_cross_loss(
            img_zs=[zi1, zi2, zio], tab_zs=[zt1, zt2, zto]
        )
        l_auto = self.c_auto * self._get_auto_loss(original=tab, augmented=[t1, t2])
    ...
    def _get_auto_loss(self, original: torch.Tensor, augmented: List[torch.Tensor]):
        loss = 0
        for a in augmented:
            loss += self.auto_loss(original, a)
    ...
    def _get_cross_loss(self, img_zs, tab_zs):
        ...
        for img_z in img_zs:
            for tab_z in tab_zs:
                loss += self.cross_loss(img_z, tab_z)
